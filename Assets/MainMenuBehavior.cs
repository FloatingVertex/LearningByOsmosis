	    ...
	    for (int i = 0; i < InputManager.Devices.Count; i++)
	    {
	        if (InputManager.Devices[i].IsAttached && InputManager.Devices[i].AnyButtonWasPressed)
	        {
	            for (int j = 0; j < players.Length; j++)
	            {
	                if (players[i] == null)
	                {
                        players[i] = new Player(InputManager.Devices[i], _colors[i],(PlayerColor)i);
	                    ...
	                    if (_countdownTime < -90 && players.Count(p=>p != null && p.Device.IsAttached) > 1 || InputManager.Devices.Count == 1)
	                    {
	                        _countdownTime = 10;
	                    }
